...
//【Note:C 语言的 % 运算结果要求和 被除数（左操作数）的符号一致。eg -1%2=-1，-3%2=-1）】
...
//一般来说 0为假 非零 为真，但若是逻辑操作符则 结果值只能是 1（真）或 0（假）。

// 7.逻辑操作符:&&逻辑与 ,||逻辑或 ,！逻辑非
// a&&b:a和b里只要有一个假（=0）结果就是假 
// a||b:a和b里只要有一个真（不等于0）结果就是真
// !a :如果操作数为真（非零），则 ! 运算结果为假（0）。如果操作数为假（零），则 ! 运算结果为真（1）。
...
//【Note：括号不是必须的,但只有括号了才等于最右边表达式，因为逗号的优先级最低】
...
int main()
{
    ...
    i = a2++ && ++b2 && d2++; // Note：因为a2是0假的,所以后面就都不算了直接出了，b2d2无变化,
    //逻辑与&&:仅是真时才会继续走(哪里假了哪里之后停)
    // i = a++||++b||d++;
    //逻辑或||:仅是假时才会继续走(哪里真了哪里之后停)
    printf("a2 = %d\n b2 = %d\n c2 = %d\nd2 = %d\n", a2, b2, c2, d2);
    ...
    printf("%zu\n", sizeof(+c00));
    printf("%zu\n", sizeof(-c00));
    // c只要参与表达式运算,就会发生整形提升,表达式 +c ,就会发生整型提升,所以 sizeof(+c) 是4个字节

    // 算术转换:当两个操作数类型不同时，等级较低的类型会被转换为等级较高的类型，然后运算在转换后的类型中进行。
    // 1. `long double`
    // 2. `double`
    // 3. `float`
    ...
    // 9. `int` ← 最低等级
    // eg1：4/1.0 强制变成浮点运算，
    // eg2: -4() sizeof(xxx),sizeof是unsigned int类(size_t类)所以-4(负数)被强制转成unsigned int类,被视为特别大的数最终 >
